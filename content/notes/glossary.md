...
(concatenacao)=
> [Concatenação](#concatenacao-de-strings) (Lógica).
> 
...
(constante-logica)=
> [Constante](#constante-logica) (Lógica).
> 
...
(delimitador-logica)=
> [Delimitador](#delimitador-logica) (Lógica).
> 
> _Delimitadores_ são {g:símbolos} dentro do {g:alfabeto} de uma {g:linguagem formal}, normalmente representados por parênteses ou colchetes, os quais são utilizados para explicitar o {g:ordenamento} de {g:operações} dentro de uma {g:fórmula}. Também são utilizados entre {g:variáveis} para dizer que estão sendo sujeitas à ação da {g:relação lógica} ou do {g:símbolo funcional} que os precede.
...
> 
> Um _predicado_ é uma {g:proposição} que "depende" de (isto é, cujo {g:valor} varia relativamente a) outro {g:símbolo}. Pensamos num {g:predicado} como sendo uma "propriedade" de um {g:indivíduo}, ou mesmo como uma {g:relação unária}.

(proposicao-logica)= 
> [Proposição](#proposicao-logica) (Lógica).
>
> Uma _proposição_ (também chamada de {g:afirmação}) é um {g:símbolo} ao qual está associado um entre possíveis valores {g:constantes}.
...
>
> Uma _relação_ é um {g:símbolo} que _depende_ de outros {g:símbolos}, chamados de {g:variáveis}. Para cada {g:relação} está associado um número não negativo, chamado de {g:aridade}: trata-se do número de {g:símbolos} ao qual a relação depende. Relações são normalmente denotadas por letras gregas, como :tex \alpha, \beta, \varphi:. 
> 
> O fato de que {g:variáveis} :tex x, y, z: se relacionam através de uma dada {g:relação} :tex \varphi: é normalmente denotado via {g:notação funcional} :tex \varphi(x, y, z, ...):

(relacao-unaria-logica)= 
> [Relação Unária](#relacao-unaria-logica) (Lógica).
>
> Uma  {g:relação} é dita ser _unária_ se ela depende de apenas uma {g:variável}. Ao aturem, {g:relações unárias} são normalmente denotadas sem o uso de {g:delimitadores lógicos}: escreve-se :tex \varphi x: ou :tex x \varphi:.

(relacao-binaria-logica)=
> [Relação Binária](#relacao-binaria-logica) (Lógica).
> 
> Uma {g:relação} é dita ser _binária_ se ela depende precisamente de duas {g:variáveis}. Normalmente, a atuação de uma {g:relação binária} é apresentada em {g:notação infixa}: :tex x \varphi y:.

# S

(sequencia-logica)=
> [Sequência](#sequencia-logica) (Lógica).
> 
> Uma _sequência_ é uma lista {g:ordenada} de {g:símbolos}.
...
> 
> O _valor_ de um {g:símbolo} é o significado associado a ele, o qual pode (ou não) depender de um {g:contexto}.
